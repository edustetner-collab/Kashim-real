  ...
  const ToggleRow = ({ label, sublabel, value, onChange }: { label: string; sublabel?: string; value: boolean; onChange: (v: boolean) => void }) => (
    <div className="flex items-center justify-between py-3 border-b border-zinc-800 last:border-0">
      ...
      <button
        ...
        className={`relative w-12 h-6 rounded-full transition-colors ${value ? 'bg-yellow-600' : 'bg-zinc-700'}`}
      >
    ...
    <div className="fixed inset-0 z-[200] overflow-y-auto bg-black/80 backdrop-blur-sm flex items-start justify-center p-4 pt-10">
      <div className="max-w-2xl w-full pb-16">
        ...
        <div className="flex gap-1 mb-5 bg-zinc-900 p-1 rounded-xl border border-zinc-800">
          {([['conta','Conta','fa-user'],['notificacoes','Notificações','fa-bell'],['parceiro','Parceiro','fa-heart']] as const).map(([key, label, icon]) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`flex-1 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all ${tab === key ? 'bg-yellow-600 text-black shadow' : 'text-zinc-500 hover:text-white'}`}
            >
          ...
          <>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 mb-4">
              <h3 className="text-white font-black uppercase italic tracking-tight mb-4 flex items-center gap-2">
                <i className="fas fa-user text-yellow-500"></i>Conta
              </h3>
              {user && (
                <div className="flex items-center gap-4 mb-4 p-4 bg-zinc-800 rounded-2xl">
                  <div className="relative shrink-0">
                    <img src={user.imageUrl} className="w-16 h-16 rounded-full border-2 border-yellow-500/30 object-cover" alt="Avatar" />
                    <button onClick={() => photoInputRef.current?.click()} disabled={photoLoading} className="absolute -bottom-1 -right-1 w-6 h-6 bg-yellow-600 hover:bg-yellow-500 rounded-full flex items-center justify-center shadow-lg transition-all active:scale-90" title="Alterar foto">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <button onClick={() => photoInputRef.current?.click()} disabled={photoLoading} className="text-yellow-600 hover:text-yellow-500 text-[10px] font-bold uppercase tracking-widest mt-1 transition-colors">Alterar foto</button>
                  </div>
                </div>
              )}
              {photoError && (
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 mb-4">
              <h3 className="text-white font-black uppercase italic tracking-tight mb-1 flex items-center gap-2">
                <i className="fas fa-lock text-yellow-500"></i>Senha de acesso
              </h3>
              ...
              {passwordView === 'idle' ? (
                ...
                <div className="flex flex-col gap-3">
                  {hasPassword && (
                    <div>
                      ...
                      <div className="relative">
                        <input type={showCurrent ? 'text' : 'password'} value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} placeholder="••••••••" className="w-full bg-zinc-800 border border-zinc-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-yellow-600 pr-12" />
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input type={showNew ? 'text' : 'password'} value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="Mínimo 8 caracteres" className="w-full bg-zinc-800 border border-zinc-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-yellow-600 pr-12" />
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input type={showConfirm ? 'text' : 'password'} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Repita a senha" className="w-full bg-zinc-800 border border-zinc-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-yellow-600 pr-12" onKeyDown={e => { if (e.key === 'Enter') handleSavePassword(); }} />
                  ...
                  <div className="flex gap-2 pt-1">
                    <button onClick={handleSavePassword} disabled={passwordLoading} className="flex-1 bg-yellow-600 hover:bg-yellow-500 disabled:opacity-50 text-black font-black py-3 rounded-2xl transition-all text-sm uppercase">
        ...
        {tab === 'notificacoes' && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
            <h3 className="text-white font-black uppercase italic tracking-tight mb-1 flex items-center gap-2">
              <i className="fas fa-bell text-yellow-500"></i>Notificações
            </h3>
            ...
            {prefs.weeklyEmail && (
              <div className="mb-3 mt-2">
                <button
                  ...
                  className={`w-full py-3 rounded-2xl text-sm font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${digestSent ? 'bg-green-600 text-white' : 'bg-zinc-800 border border-zinc-700 text-zinc-300 hover:text-white hover:border-yellow-600'}`}
                >
                  {digestSending ? <><i className="fas fa-circle-notch animate-spin"></i> Enviando...</>
                  : digestSent ? <><i className="fas fa-check"></i> Enviado!</>
                  : <><i className="fas fa-paper-plane text-yellow-500"></i> Enviar resumo agora</>}
                </button>
              </div>
            )}
            ...
            {prefs.pushEnabled && (
              <div className="mb-4">
                ...
                <div className="flex justify-between text-[9px] text-zinc-600 font-mono mt-0.5">
                  <span>50%</span><span className="text-yellow-500 font-black">{prefs.alertThresholdPct}%</span><span>100%</span>
                </div>
              </div>
            )}
            ...
            {prefs.recurringReminderDay > 0 && (
              <div className="mt-2 mb-2">
                ...
                />
                <div className="flex justify-between text-[9px] text-zinc-600 font-mono mt-0.5">
                  <span>Dia 1</span><span className="text-yellow-500 font-black">Dia {prefs.recurringReminderDay}</span><span>Dia 28</span>
                </div>
              </div>
            )}
